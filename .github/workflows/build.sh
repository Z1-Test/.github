...
TEST_OUT=$(query <<EOF | jq -r .container.from.withDirectory.withWorkdir.withSecretVariable.withSecretVariable.withSecretVariable.withSecretVariable.withSecretVariable.withEnvVariable.withExec.stdout
{
  container {
    from(address: "ghcr.io/staytunedllp/node-gh:latest") {
                ...
                withSecretVariable(name: "CLIENT_PAYLOAD", secret: "$S_CLIENT_PAYLOAD") {
                  withSecretVariable(name: "EVENT_TYPE", secret: "$S_EVENT_TYPE") {
                     withEnvVariable(name: "CACHE_BUSTER", value: "$(date)") {
                      withExec(args: [ "trigger-event.js"]) {
                        stdout
                      }
                    }
                  }
...
# alpine=$(dagger query <<EOF | jq -r .container.from.withDirectory.withEntrypoint.stdout
# {
#     container {
#     from(address: "ghcr.io/staytunedllp/node-gh:latest") {
#         withDirectory(path:"/app", directory:"$S_SOURCE_DIR") {
